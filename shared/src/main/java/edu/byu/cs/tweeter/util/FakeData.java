...
public class FakeData {
    ...
    private void generateFakeStatuses() {
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < fakeUsers.size(); ++j) {
                ...
                Status status = new Status(post, sender, System.currentTimeMillis(), datetime, urls, mentions);
    ...
    public Pair<List<Status>, Boolean> getPageOfStatus(String lastStatsUserAlias, String date, int limit) {
        ...
        if (lastStatsUserAlias != null) {
            for (int i = 0; i < fakeStatuses.size(); ++i) {
                Status curStatus = fakeStatuses.get(i);
                if (curStatus.getUser().getAlias().equals(lastStatsUserAlias) &&
                        curStatus.getDate().equals(date)) {
                    index = i + 1;
                    break;
                }
            }
